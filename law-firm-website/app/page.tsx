...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* ─── Navbar ───────────────────────────────────── */}
      <header className="fixed top-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-lg border-b border-slate-200/70 shadow-sm">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <Scale className="text-blue-700" size={22} />
            <span className="font-bold text-lg tracking-tight text-slate-900">
              EagleNest <span className="text-blue-700">Legal</span>
            </span>
          </div>
          ...
          <div className="hidden md:flex items-center gap-3">
            <a href={CLIENT_PORTAL_URL} className="text-sm font-semibold text-blue-700 hover:text-blue-900 transition-colors">
              Client Portal
            </a>
            <a
              href={CLIENT_PORTAL_URL}
              className="flex items-center gap-1.5 bg-blue-700 hover:bg-blue-800 text-white text-sm font-semibold px-4 py-2 rounded-lg shadow-sm transition-all"
            >
        ...
        {/* Mobile menu */}
        {mobileOpen && (
          <div className="md:hidden bg-white border-t border-slate-100 px-6 py-4 space-y-3 text-sm font-medium text-slate-700">
            <a href="#practice-areas" className="block" onClick={() => setMobileOpen(false)}>Practice Areas</a>
            <a href="#about" className="block" onClick={() => setMobileOpen(false)}>About</a>
            <a href="#contact" className="block" onClick={() => setMobileOpen(false)}>Contact</a>
            <a href={CLIENT_PORTAL_URL} className="block text-blue-700 font-bold">Client Portal →</a>
          </div>
        )}
      </header>
      ...
      >
        <div className="absolute inset-0 bg-gradient-to-br from-slate-950/85 via-blue-950/75 to-slate-900/80" />
        <div className="absolute bottom-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-amber-500/50 to-transparent" />

        <div className="relative z-10 text-center px-6 max-w-5xl mx-auto pt-20">
          <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-md border border-white/20 text-amber-300 text-xs font-bold uppercase tracking-widest px-5 py-2 rounded-full mb-8 shadow-lg">
            <Scale size={12} /> Serving Swat Valley Since 2005
          </div>
          ...
          <p className="text-lg md:text-xl text-slate-300 max-w-2xl mx-auto leading-relaxed mb-12">
            ...
            We combine deep local expertise with modern legal strategy.
          </p>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {practiceAreas.map((area) => {
              ...
              return (
                <div key={area.title} className={`group bg-white rounded-2xl p-8 border border-slate-100 ${area.border} hover:shadow-xl hover:-translate-y-2 transition-all duration-300 cursor-default`}>
                  <div className={`w-14 h-14 ${area.bg} rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300`}>
      ...
      {/* ─── Stats ────────────────────────────────────── */}
      <section id="about" className="py-20 px-6 bg-gradient-to-br from-blue-900 to-slate-900 text-white">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="mt-16 max-w-2xl mx-auto text-center">
            <p className="text-slate-400 text-lg leading-relaxed">
              EagleNest Legal has been the trusted legal partner for families, businesses, and individuals
              throughout Swat Valley and Khyber Pakhtunkhwa for over two decades.
              Our team brings rigorous legal expertise combined with an intimate understanding of local courts and customs.
            </p>
          </div>
        </div>
      </section>
      ...
      <footer className="bg-slate-950 text-slate-400 py-12 px-6">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-white text-lg">EagleNest Legal</span>
          </div>
          <p className="text-sm text-center">© {new Date().getFullYear()} EagleNest Legal, Swat. All rights reserved.</p>
