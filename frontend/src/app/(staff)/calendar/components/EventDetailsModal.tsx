...
export function EventDetailsModal({ isOpen, onClose, event, onEventDeleted }: EventDetailsModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 dark:bg-slate-900/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200 transition-colors border border-transparent dark:border-slate-800">
        <div className={`p-6 border-b ${colors.split(' ')[0]} ${colors.split(' ')[2]}`}>
          <div className="flex justify-between items-start gap-4">
            <div>
              <span className={`inline-block px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider mb-3 bg-white/60 dark:bg-white/10 ${colors.split(' ')[1]}`}>
                {event.event_type}
              </span>
              <h2 className="text-2xl font-bold text-slate-900 dark:text-white leading-tight">{event.title}</h2>
            </div>
            <button onClick={onClose} className="p-2 bg-white/50 dark:bg-slate-800/50 hover:bg-white/80 dark:hover:bg-slate-800/80 rounded-full transition-colors shrink-0">
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            {event.case_number && (
              <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-100 dark:border-slate-700">
                ...
                {event.case ? (
                  ...
                  <div className="font-bold text-slate-900 dark:text-white">{event.case_number}</div>
                )}
              </div>
            )}
            ...
            {event.client_name && (
              <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-100 dark:border-slate-700">
                ...
                {event.client ? (
                  ...
                  <div className="font-bold text-slate-900 dark:text-white">{event.client_name}</div>
                )}
              </div>
            )}
            ...
            {event.advocate_name && (
              <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-100 dark:border-slate-700">
                ...
                <div className="font-bold text-slate-900 dark:text-white">{event.advocate_name}</div>
              </div>
            )}
            ...
            {event.judge_name && (
              <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-100 dark:border-slate-700">
                ...
                <div className="font-bold text-slate-900 dark:text-white">{event.judge_name}</div>
              </div>
            )}
          </div>
          ...
          {event.description && (
            <div>
              ...
              <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl text-sm text-slate-700 dark:text-slate-300 border border-slate-100 dark:border-slate-700 whitespace-pre-wrap">
                {event.description}
              </div>
            </div>
          )}
        ...
        <div className="p-4 border-t border-slate-100 dark:border-slate-800 flex justify-end gap-3 bg-slate-50/50 dark:bg-slate-900">
          {canDelete && (
            <button 
              ...
              className="px-4 py-2 text-rose-600 dark:text-rose-400 font-medium hover:bg-rose-50 dark:hover:bg-rose-900/30 rounded-lg transition-colors mr-auto"
            >
              Delete Event
            </button>
          )}
          <button 
            onClick={onClose}
            className="px-6 py-2 bg-slate-900 dark:bg-blue-600 hover:bg-slate-800 dark:hover:bg-blue-700 text-white font-medium rounded-xl transition-colors shadow-md"
          >
            Close
          </button>
        </div>
