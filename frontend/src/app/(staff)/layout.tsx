...
import { Menu, LogOut, UserCircle, ChevronDown, Search, Plus, Bell, CalendarDays, CheckCircle2, Clock, FileText } from 'lucide-react';
...
export default function StaffLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen overflow-hidden w-full">
      ...
      />
      <main className="flex-1 flex flex-col overflow-y-auto relative bg-[#F8FAFC] min-w-0">
        {/* ── Top Bar ──────────────────────────────────────────── */}
        <div className="min-h-[88px] bg-white/80 backdrop-blur-xl border-b border-blue-100/50 flex items-center px-4 md:px-8 pt-4 pb-2 sticky top-0 z-20 gap-3 shadow-[0_4px_24px_-12px_rgba(37,99,235,0.08)]">
          ...
          <div className="hidden md:flex flex-col flex-1 ml-4">
            <h2 className="text-[1.1rem] font-bold text-slate-900 flex items-center gap-2">
          ...
          <div className="flex items-center gap-3">
            <div className="relative hidden lg:block mr-2">
              ...
              <input type="text" placeholder="Search anything..." className="pl-10 pr-4 py-2.5 w-[280px] bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all placeholder:text-slate-400 shadow-sm" />
            </div>
            ...
            <button 
              ...
              className="p-2.5 text-slate-600 hover:bg-slate-50 rounded-xl transition-colors relative border border-slate-200 bg-white shadow-sm mr-2"
            >
            ...
            <div className="relative shrink-0" ref={notificationsRef}>
              <button 
                ...
                className={`p-2.5 text-slate-600 rounded-xl transition-colors relative border shadow-sm ${notificationsOpen ? 'bg-slate-50 border-blue-200 shadow-blue-100' : 'bg-white border-slate-200 hover:bg-slate-50'} mr-2`}
              >
                <Bell size={20} />
                {unreadCount > 0 && (
                ...
                <div className="absolute right-0 top-full mt-2 w-80 bg-white rounded-2xl shadow-xl border border-slate-100 py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-150">
                  ...
                  <div className="flex flex-col max-h-[350px] overflow-y-auto px-2 custom-scrollbar">
                    {notifications.length === 0 ? (
                      ...
                      notifications.map((n) => (
                        <div key={n.id} onClick={() => markAsRead(n.id, n.is_read)} className={`flex gap-3 p-2 rounded-xl hover:bg-slate-50 transition-colors cursor-pointer ${!n.is_read ? 'bg-blue-50/50' : ''}`}>
                          ...
                          <div className="flex-1">
                            <p className={`text-sm leading-tight mb-0.5 ${!n.is_read ? 'font-bold text-slate-900' : 'font-semibold text-slate-700'}`}>{n.title}</p>
                            <p className="text-[11px] font-medium text-slate-500">{n.message}</p>
                          </div>
          ...
          <div className="relative shrink-0" ref={profileRef}>
            <button
              ...
              className="flex items-center gap-2.5 pl-3 pr-2.5 py-1.5 rounded-xl border border-blue-100 bg-white hover:bg-blue-50/50 hover:border-blue-200 transition-all duration-200 shadow-[0_2px_8px_rgba(0,0,0,0.02)]"
            ...
            >
              ...
              <div className="hidden sm:block text-left mr-2">
                <p className="text-[11px] text-slate-500 leading-tight font-medium uppercase tracking-wider mb-0.5">{user.role}</p>
