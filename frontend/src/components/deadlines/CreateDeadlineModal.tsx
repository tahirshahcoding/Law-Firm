    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl my-8">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <h2 className="text-lg font-bold text-slate-800">Create Deadline</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-600 focus:border-transparent transition-all outline-none"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-5">
            ...
            <div className="relative" ref={caseMenuRef}>
              <label className="block text-sm font-semibold text-slate-700 mb-1.5">Related Case</label>
              <div 
                className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl cursor-pointer flex justify-between items-center"
                onClick={() => setCaseMenuOpen(!caseMenuOpen)}
              >
                <span className={`truncate ${selectedCaseObj ? 'text-slate-900' : 'text-slate-400'}`}>
              ...
              {caseMenuOpen && (
                <div className="absolute z-10 w-full mt-1 bg-white border border-slate-200 rounded-xl shadow-lg max-h-60 overflow-y-auto">
                  ...
                  <div className="p-1">
                    <div 
                      className={`px-3 py-2 text-sm rounded-lg cursor-pointer flex items-center justify-between ${formData.case === '' ? 'bg-blue-50 text-blue-700' : 'hover:bg-slate-50'}`}
                      onClick={() => { setFormData({...formData, case: ''}); setCaseMenuOpen(false); }}
                    >
                      <span className="text-slate-500 italic">No Case</span>
                      {formData.case === '' && <Check size={14} />}
                    </div>
                    {filteredCases.map((c) => (
                      <div 
                        key={c.id}
                        className={`px-3 py-2 text-sm rounded-lg cursor-pointer flex items-center justify-between ${formData.case === c.id ? 'bg-blue-50 text-blue-700' : 'hover:bg-slate-50 text-slate-700'}`}
                      ...
                      >
                        ...
                        {formData.case === c.id && <Check size={14} className="shrink-0" />}
                      </div>
                    ))}
                    {filteredCases.length === 0 && (
                      <div className="px-3 py-4 text-center text-sm text-slate-500">No cases found</div>
                    )}
                  </div>
                </div>
              )}
            ...
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1.5">Deadline Type</label>
              <select
                name="deadline_type"
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={handleChange}
                  required
                  className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-600 outline-none"
                />
              </div>
            </div>
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-2">Reminders</label>
            <div className="flex gap-4">
              {['7', '3', '1'].map(days => (
                <label key={days} className="flex items-center gap-2 cursor-pointer">
                  ...
                  />
                  <span className="text-sm text-slate-700">{days} Days Before</span>
                </label>
              ))}
            </div>
          </div>
